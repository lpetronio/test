...
const ConfigFiguresPop = {
    ...
    height: d3.selectAll(".plot-wrapper").node().clientHeight * .65,
    color: {
        type: "single",
        fill: true
    },
...
const ConfigTower = {
   ...
   // height: d3.select("#plot-wrapper").node().clientHeight, 
    ...
    padding: {top:0, right: d3.selectAll(".plot-wrapper").node().clientWidth*.35, bottom:20, left: d3.selectAll(".plot-wrapper").node().clientWidth*.425}
...
const ConfigFigures = {
   ...
   // padding: {top:0, right: 20, bottom:100, left: d3.select("#plot-wrapper").node().clientWidth*.4}
    padding: {top:0, right: d3.selectAll(".plot-wrapper").node().clientWidth*.35, bottom:120, left: d3.selectAll(".plot-wrapper").node().clientWidth*.425}
...
function init(  tConfig = ConfigTower, 
    ...
    var tData = Array.from(data).filter((n, i) => n.percentile == 50); // used to be 51
    ...
    var fConfig = Configuration.makeFigureData(fConfig, undefined) // can take data. if so, will need a parser
 //   var fpConfig = Configuration.makeFigureData(fpConfig, undefined) // can take data. if so, will need a parser
console.log("fpConfig", fpConfig)

    PsPlots.render(configPsPlots)
...
//    d3.select(`#${tConfig.rootId}-${tConfig.parentId}`).attr("opacity", scrollUtils.fadeIn(e.progress))  
    ...
    //     d3.select(`#${fConfig.rootId}-${fConfig.parentId}`).attr("opacity", scrollUtils.fadeOut(e.progress))  
    //     d3.select(`#${tConfig.rootId}-${tConfig.parentId}`).attr("opacity", scrollUtils.fadeOut(e.progress))  
    //     d3.select(`#psplot-gradientplot`).attr("opacity", scrollUtils.fadeOut(e.progress))  
    //     d3.select(`#gradientplot-triangle-50`).style("opacity", scrollUtils.fadeOut(e.progress))
    //     d3.selectAll(".gradientplot-tick").attr("opacity", scrollUtils.fadeOut(e.progress)) 
    //     d3.selectAll(".gradientplot-label-riskbin").attr("opacity", scrollUtils.fadeOut(e.progress)) 
    ...
    population
        .on('enter', function(e){
            ...
            // if (e.scrollDirection == "FORWARD"){
            //     console.log(e)
            //     d3.select(`#${fpConfig.rootId}-${fpConfig.parentId}`).transition().duration(scrollUtils.transitionDuration).attr("opacity", 1) 
            //     d3.select(`#${tpConfig.rootId}-${tpConfig.parentId}`).transition().duration(scrollUtils.transitionDuration).attr("opacity", 1) 
            // } else {
            //     d3.select(`#${fpConfig.rootId}-${fpConfig.parentId}`).transition().duration(scrollUtils.transitionDuration).attr("opacity", 0) 
            //     d3.select(`#${tpConfig.rootId}-${tpConfig.parentId}`).transition().duration(scrollUtils.transitionDuration).attr("opacity", 0) 
            // }
